...
from runway import (
    RunwayCurvePoint, absent_runway, shift_income_scenario_dates,
)
...
def run_runway_sweep(cfg: Dict, shock_dates: List[_date],
    ...
    for new_date in shock_dates:
        ...
        rw = best.get('runway', absent_runway())
...
def main():
    ...
    # Issue #232 slice 3 (DP#25/DP#9): the exploration reports' rendering lives
    # in the reporting layer. Import the printers HERE rather than at module
    # scope, so this module does not re-export them under the old private names
    # -- the caller renders, the seam only ranks.
    from output_plugins import (
        _print_borrow_to_invest_report,
        _print_decumulation_shortfall_report,
        _print_income_scenario_report,
        _print_ltv_exploration,
        _print_property_funding_report,
        _print_structure_report,
    )
