...
def test_nonreg_management_fee_is_deductible():
    ...
    ws.management_fee_deductible = 500.0
    ctx = RuleContext(config=config, use_readvanceable=False, heloc_rate=0.0,
                      non_reg_yield_rate=0.06, primary_retired=False)
    fired = apply_sm_interest(ws, ctx)
    assert fired
    assert ws.sm_interest_deduction == 500.0
