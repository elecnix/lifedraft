...
class TestRESPChildProvince:
    ...
    def test_quebec_province_eligible_for_qesi(self):
        ...
        child = RESPChild(grant_history=None, name="test", birth_year=2010, province='quebec')
    ...
    def test_ontario_province_not_eligible_for_qesi(self):
        ...
        child = RESPChild(grant_history=None, name="test", birth_year=2010, province='ontario')
    ...
    def test_bc_province_not_eligible_for_qesi(self):
        ...
        child = RESPChild(grant_history=None, name="test", birth_year=2010, province='bc')
        assert child.is_quebec_resident_in(2026) is False

    def test_qc_abbreviation_works(self):
        """QC abbreviation should be recognized as Quebec."""
        child = RESPChild(grant_history=None, name="test", birth_year=2010, province='QC')
        assert child.is_quebec_resident_in(2026) is True
    ...
    def test_is_quebec_resident_false_sets_ontario(self):
        ...
        child = RESPChild(grant_history=None, name="test", birth_year=2012, province='ontario')
        assert child.is_quebec_resident_in(2026) is False
    ...
    def test_is_quebec_resident_true_from_province(self):
        ...
        child = RESPChild(grant_history=None, name="test", birth_year=2012, province='quebec')
        assert child.is_quebec_resident_in(2026) is True
...
class TestQESIEligibilityFromProvince:
    ...
    def test_quebec_child_gets_qesi(self):
        ...
        child = RESPChild(grant_history=None, name="child_a", birth_year=2017, province='quebec')
    ...
    def test_ontario_child_no_qesi(self):
        ...
        child = RESPChild(grant_history=None, name="child_b", birth_year=2019, province='ontario')
        result = self.calc.calculate_qesi(2500, child, 2026, 150000)
        assert result['total_qesi'] == 0
...
class TestAdapterCreateRespChild:
    """Test that CanadaAdapter.create_resp_children uses the province (#295:
    one shared constructor, create_resp_child is deleted)."""

    def test_adapter_creates_with_province(self):
        """A child entry's province is honoured."""
        from countries.canada.adapter import CanadaAdapter
        from simulation_config import SimulationConfig

        config = SimulationConfig(children=[
            {'name': 'child_a', 'birth_year': 2017, 'province': 'ontario'}])
        adapter = CanadaAdapter(config)
        (child,) = adapter.create_resp_children(config)
        assert child.province == 'ontario'
        assert child.is_quebec_resident_in(2026) is False

    def test_adapter_default_province_quebec(self):
        """Without a child province, the household province is used."""
        from countries.canada.adapter import CanadaAdapter
        from simulation_config import SimulationConfig

        config = SimulationConfig(children=[{'name': 'Test', 'birth_year': 2010}])
        assert config.province == 'quebec'
        adapter = CanadaAdapter(config)
        (child,) = adapter.create_resp_children(config)
        assert child.province == 'quebec'
        assert child.is_quebec_resident_in(2026) is True
