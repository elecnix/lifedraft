...
def test_example_without_the_block_still_validates():
    ...
    cfg = ic.map_to_internal(doc) if hasattr(ic, "map_to_internal") else None
    if cfg is not None:
        assert cfg.get("zev_purchases") == []


def test_declared_acquisition_validates():
    ic.validate_contract(_example_with([OPEN_WINDOW_BEV]))
...
def test_schema_rejects_a_lease_without_a_term():
    ...
    with pytest.raises(Exception):
        ic.validate_contract(_example_with([bad]))
...
def test_adapter_refuses_a_duplicate_id():
    ...
    with pytest.raises(ic.ContractAdaptationError, match="more than once"):
        ic._map_zev_purchases(doc)


def test_adapter_refuses_a_lease_without_a_term():
    doc = {"zev_purchases": [dict(OPEN_WINDOW_BEV, is_lease=True)]}
    with pytest.raises(ic.ContractAdaptationError, match="lease_term_months"):
        ic._map_zev_purchases(doc)


def test_adapter_refuses_a_phev_without_a_range():
    doc = {"zev_purchases": [dict(OPEN_WINDOW_BEV, propulsion="phev")]}
    with pytest.raises(ic.ContractAdaptationError, match="electric_range_km"):
        ic._map_zev_purchases(doc)
...
def test_adapter_maps_every_field():
    ...
    mapped = ic._map_zev_purchases({"zev_purchases": [entry]})
    assert len(mapped) == 1
